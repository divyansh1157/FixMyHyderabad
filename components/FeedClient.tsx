...
export default function FeedClient({
  ...
  const stats = [
    ...
    {
      label: t('statsAreas'),
      value: new Set(reports?.map((r) => r.area_name) ?? []).size,
      emoji: '📍',
    },
  ];
      ...
      <aside className="hidden lg:flex flex-col gap-4 sticky top-24 self-start">
        ...
        <div className="grid grid-cols-1 gap-2">
          {stats.map(({ label, value, emoji }) => (
            <div
              key={label}
              className="bg-white border border-slate-200 rounded-xl px-4 py-3 flex items-center justify-between shadow-sm"
            >
        ...
        <p className="text-[10px] text-[#1A1208]/30 leading-relaxed px-1">
          Data updates in real-time via Supabase. Issues are routed to GHMC for
          municipal action.
        </p>
      </aside>
