...
export default async function RootLayout({ children, params }: LayoutProps) {
  ...
  return (
    <html lang={locale} dir={isRtl ? 'rtl' : 'ltr'}>
      <body
        className={`${inter.variable} ${notoUrdu.variable} antialiased ${isRtl ? 'font-urdu' : 'font-sans'}`}
      >
        <NextIntlClientProvider locale={locale} messages={messages}>
          ...
          <header className="sticky top-0 z-40 bg-white/90 backdrop-blur-md border-b border-gray-200 shadow-sm">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
              <Link
                href={`/${locale}`}
                className={`flex items-center gap-2.5 shrink-0 ${isRtl ? 'flex-row-reverse' : 'flex-row'}`}
              >
                <span className="text-2xl">🏙️</span>
                <div className={isRtl ? 'text-right' : 'text-left'}>
                  <p
                    className="font-extrabold text-lg tracking-tight leading-none text-gray-900"
              ...
              >
                ...
                <Link
                  href={`/${locale}/report`}
                  className="bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold px-4 py-2 rounded-full shadow-md transition-all active:scale-95"
                >
                  {nav.reportIssue}
                </Link>
              </div>
